...
async function requestJson<T>(
  ...
  if (isLoginResponse(response, contentType, text)) {
...
async function login(employeeNo = chainViewEmployeeNo) {
  ...
  if (!token) {
    throw new ChainViewApiError({
      authRequired: true,
      message: "ChainView 로그인 CSRF 토큰을 찾지 못했습니다.",
      status: 0,
    });
  }
...
function isLoginResponse(response: Response, contentType: string, body: string) {
  return (
    response.type === "opaqueredirect" ||
    response.status === 0 ||
