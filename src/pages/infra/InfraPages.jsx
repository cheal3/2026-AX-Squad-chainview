      ...
      <main className="main infra-page">
        ...
        <div className="card">
          <table className="tbl infra-table">
            <thead>
              <tr>
                <th>출발 서비스</th><th>대상 서비스</th><th>관계유형</th><th>필수</th><th>상태</th><th>설명</th><th className="col-actions">관리</th>
              </tr>
            </thead>
            <tbody>
              ...
              <div className="modal__body">
                <div className="form-section">
                  ...
                  <div className="form-grid">
                    <div className="form-row"><label>source 인프라<span className="req">*</span></label><select value={modal.form.sourceInfraNodeId} onChange={(event) => updateModalField("sourceInfraNodeId", event.target.value)} disabled={modal.mode === "edit"}>{nodes.map((node) => <option key={node.infraNodeId} value={node.infraNodeId}>{node.nodeCode} {node.nodeName}</option>)}</select><span className="help">영향을 주는 쪽</span></div>
                    <div className="form-row"><label>target 인프라<span className="req">*</span></label><select value={modal.form.targetInfraNodeId} onChange={(event) => updateModalField("targetInfraNodeId", event.target.value)} disabled={modal.mode === "edit"}>{nodes.map((node) => <option key={node.infraNodeId} value={node.infraNodeId}>{node.nodeCode} {node.nodeName}</option>)}</select><span className="help">영향을 받는 쪽</span></div>
